...
// Refernce cross track error, psi error and we want the car to go fast.. :)

double ref_cte      = 0;
double ref_epsi     = 0;
double ref_v        = 80; // Tweaked and now goes upto 76 mph
...
class FG_eval
{
 public:
  ...
  {
    // Cost is stored in the first element of fg, and it is a function of
    // cross-track error, error in psi in addition to sharp / sudden turns
    //
    // Used the equations from the solution to the quiz directly..
      ...
      int k_delta   = 100;
      int k_a       = 1; // speed control.. higher value penalizes the speed
    ...
    if (check_max_speed)
    {
        cout << "Input max_speed : " << endl;
        cin >> ref_v ;
        cout << endl;
        check_max_speed = false;
    }
    ...
    // Minimize the use of actuators. Steering control helps around tight corners.
    for (int i = 0; i < N - 1; i++)
    {
        fg[0] += k_delta * CppAD::pow(vars[delta_start + i], 2);
        fg[0] += k_a * CppAD::pow(vars[a_start + i], 2);
    }
        
    // Minimize the value gap between sequential actuations. Penalize sudden changes.
    for (int i = 0; i < N - 2; i++)
    {
        fg[0]  += k_ds * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
        fg[0]  += k_as * CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }
      
    //
    // Set up the constraints
    //
    // Initial constraints - we add 1 to each starting index as cost is being located
    // at index 0 of fg. This bumps up the position of all other variables
    //
    ...
    for (int i = 0; i < N - 1; i++)
    {
        ...
        */
        ...
        //
        // We constrain our values in the next timestep to be 0
        // Model equations as we had seen in the class
        //
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs)
{
  ...
  // Now return the first actuator values. The variables can be accessed with
  // `solution.x[i]`.
  //
