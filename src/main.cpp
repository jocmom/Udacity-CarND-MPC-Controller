    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2')
    {
      ...
      if (s != "")
      {
        ...
        if (event == "telemetry") {
          ...
          // Solve for the best possible next actuator values
            
          auto vars = mpc.Solve(state, coeffs);
            
          //Display the waypoints/reference line for visual debugging, Yellow line
          vector<double> next_x_vals;
          vector<double> next_y_vals;
            
          double poly_inc   = 0.7; // x distance
          int num_points    = 20;   // Number of points we want to see
          ...
          // Remember what was returned.. after the first 2 entries containing steering value and
          // throttle, rest is alternating a and y values.. x as even entries and y as odd entries
          {
              if (i%2 == 0)
              {
